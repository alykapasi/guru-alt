...
Repository snapshot: `7aefb12c2b6e7d2904ad0563b564475aadd8b9a9`
...
The recorded retrieval gate failure (S76) and evidence semantics (S54/S56) must stay visible even
where their surrounding infrastructure is already implemented.
...
[completed work](#completed-and-consolidated-work); S33 and P10 closed earlier. What remains for
this workstream is operating configuration, not code: provisioning a real identity provider
application and production mail are part of S60, and no deployment has yet talked to a real Clerk
endpoint.
...
| --- | --- | --- | --- | --- |
| S01 | Goal-specific achievement and closure | Partial | Replace the fixed ability/uncertainty test with V02's conservative estimate, evidence freshness, independent demonstrations across time, and explicit learner closure. Distinguish current confidence from historical achievement; closure must not fabricate ability evidence. Depends on S14/S18/S56. | [Goal decisions](V0_DECISIONS.md#goal-model-interpretation-to-resolve-in-the-focused-design), [current mastery rule](../app/services/lesson_plan.py) |
| S11 | Learner-controlled prerequisite detours | Partial | Automatic, bounded detours exist. In exploration, propose substantial detours; in guided mode, allow automatic detours. Add accept/skip behavior and return to the original objective, including when the detour disproves the suspected gap. Use the explicit guidance settings in S02. | [Plan service](../app/services/lesson_plan.py), [detour tests](../tests/test_prerequisite_detour.py) |
| S14 | Independent delayed retention and transfer evidence | Partial | Fresh-item selection and exposure summaries exist. Add delayed independent probes and genuinely different applications, then make goal status consume that evidence. Two item IDs or a time span alone do not establish transfer or retention. | [Item selection](../app/services/assessment.py), [evidence summaries](../app/learning/mastery.py), [exposure tests](../tests/test_item_exposure.py) |
| S23 | Graph integrity under concurrent edits | Partial | Sequential cycle validation and deterministic handling of existing cycles exist. Serialize conflicting graph edits and test concurrent cycle creation. Surface existing conflict/repair information in the learner workflow. | [Graph API](../app/api/v1/knowledge.py), [graph service](../app/services/knowledge.py), [validation tests](../tests/test_graph_validation.py) |
| S24 | Confirmed cross-subject concept transfer | Partial | Canonical-name identity and external-prerequisite reporting exist. Add confirmed equivalence, preserve the original evidence context, and require a short confirmation for a materially new application. Plan traversal must handle confirmed external prerequisites; names alone must not merge estimates. | [Concept service](../app/services/knowledge.py), [plan service](../app/services/lesson_plan.py), [concept tests](../tests/test_knowledge_concepts.py) |
| S34 | Concurrency-safe evidence and retries | Partial | Attempt deduplication and durable turn locks exist. Serialize distinct attempts updating the same existing KC state; the current read/modify/write path lacks a row lock or version check. Test that race on separate connections and preserve attempt identity across chat/workflow retries. Existing duplicate-attempt coverage does not establish distinct-attempt safety. | [Mastery updates](../app/learning/mastery.py), [turn lock](../app/services/turn_lock.py), [cross-connection tests](../tests/test_cross_connection.py) |
| S52 | Pause, resume, and skip practice explicitly | Partial | Conversation phases and turn lifecycle are persisted. Implement V07's side-discussion pause/resume/skip behavior with preserved attempt state; current mode-based bypass/resume rules do not provide that control. | [Chat routing](../app/api/v1/chat.py), [workflow service](../app/services/workflow.py), [phase tests](../tests/test_conversation_phase.py) |
| S54 | Flashcard reveal and self-rating | Partial | Public item schemas already withhold secret answers. Add the accepted think → reveal → self-rate interaction without exposing grading keys before answering other item types. Pair it with S56 so self-ratings update retention only. | [Public presentation](../app/learning/item_presentation.py), [item schemas](../app/schemas/assessment.py), [practice panel](../frontend/src/components/lessons/ItemPanel.tsx), [privacy tests](../tests/test_assessment_privacy.py) |
| S56 | Evidence kinds and reproducible grading history | Partial | Versioned estimator replay exists, and admin activity is separated from learner evidence. Self-rated flashcards still reach the ordinary ability-update path: record evidence kind on the server and separate retention-only ratings throughout updates, analytics, and replay. Give declared conversational checks explicit rubrics and difficulty targets; record immutable item/rubric/prompt provenance and assistance context. Legacy events without sufficient data remain explicitly unreplayable. | [Assessment service](../app/services/assessment.py), [mastery events](../app/learning/mastery.py), [item/rubric models](../app/models/assessment.py), [replay tests](../tests/eval/test_datasets_replay.py) |
| S63 | Show the complete goal beyond the planning window | Partial | Full objectives and horizon extension are implemented. Render the API's objective/deferred counts in the lesson-plan UI so completing the current window does not look like finishing the entire goal. | [Plan service](../app/services/lesson_plan.py), [plan panel](../frontend/src/components/lessons/LessonPlanPanel.tsx), [plan tests](../tests/test_lesson_plan.py) |

### 3. Source scope, teaching quality, and retrieval

| ID | Item | Status | Next step or closure | Evidence |
| --- | --- | --- | --- | --- |
| S26 | Consistent source scope and sources-only mode | Partial | Subject filtering exists, but content generation still opts into unassigned sources. Apply V05 across all generation paths: explicit selected scope, no silent unrelated/unassigned material, an explicit sources-only setting, and clear attribution when general knowledge supplements sources. | [Content retrieval](../app/services/content.py), [scope tests](../tests/test_source_scope.py), [content tests](../tests/test_content.py) |
...
| S28 | Explain source support and insufficiency | Partial | Empty-context prompts, contradiction instructions, grounding counts, and an optional support checker exist. Show source coverage to learners and handle partly insufficient/conflicting material consistently. Sources-only policy belongs to S26; checker accuracy belongs to S59. | [Content generation](../app/services/content.py), [support checker](../app/learning/citation_support.py), [support tests](../tests/test_citation_support.py) |
...
| S76 | Retrieval correctness and corpus-scale behavior | Open | Reproduce, explain, and fix the recorded retrieval gate failure before treating the backend gate as green. Separately compare exact and index-based retrieval on representative permitted corpora before choosing a performance tradeoff. Preserve the distinction between the unresolved failure and the historical synthetic measurements below. | [Retrieval gate](../tests/eval/test_eval.py), [retrieval query](../app/rag/retrieval.py), [recall/plan experiment](../tests/eval/retrieval/recall.py) |
...
checks; studies using ability or downstream learning outcomes also depend on S54/S56. Adding these
entries records the implementation scope, without claiming live evaluation or rollout has occurred.

| ID | Item | Status | Next step or closure | Evidence |
| --- | --- | --- | --- | --- |
| S78 | Jev decision contract and isolated adapter | Proposed | Phase 1: add a provider-neutral `DecisionClient`, deterministic fake, isolated TypeSafe transport, and disabled-by-default configuration under `app/llm/`. Validate the named three-way choice, probabilities, model/schema identity, missing answers, errors, deadlines, and cancellation. Disabled mode makes no candidate call. Use invented stateless fixtures; keep credentials and raw state out of logs. Recheck the vendor contract before implementation; a live smoke run needs its own budget. | [Phase 1](jev-implementation-plan.md#phase-1--verify-a-stateless-synthetic-contract), [contract design](jev-architecture.md#minimal-contracts-and-personalization), [current LLM boundary](../app/llm/) |
| S79 | Offline answer-intent comparison | Proposed | Phase 2: compare FAST and Jev on identical, independently reviewed attempt/deferral/withdrawal cases with separate development and held-out sets. Build deterministic fixtures/reporting first. Measure false attempts, attempt recall, false withdrawals, abstentions, failures, response quality, latency, and known/unknown cost. Version data and policy; define tolerances before held-out evaluation. Inferior or inconclusive results leave Jev disabled. Complements S18/S59. | [Phase 2](jev-implementation-plan.md#phase-2--compare-answer-intent-offline), [baseline classifier](../app/learning/conversation_evidence.py), [baseline tests](../tests/test_conversation_evidence.py) |
| S80 | Jev learner-data eligibility and provider controls | Proposed | Phase 3: verify authorization, minimal question/reply scope, admin exclusion, and deletion/suppression through actual callers before learner export. Resolve provider agreement, retention/deletion, logging, and applicable account settings. Reuse S25/S26/S31/S61 and P10: private assessment ownership and `_resolve_check`'s scoped `get_item_for` already exist. Do not reopen those repairs from the older plan. Ability/outcome studies additionally require S54/S56; a read-only intent shadow need not wait for unrelated evidence work. | [Phase 3](jev-implementation-plan.md#phase-3--prerequisite-learner-evidence-and-privacy-fixes), [current check resolution](../app/services/chat.py), [assessment privacy tests](../tests/test_assessment_privacy.py), [privacy design](jev-architecture.md#privacy-and-lifecycle) |
| S81 | Bounded shadow intent execution | Proposed | Phase 4: add a supervised, best-effort shadow alongside FAST after S78–S80. It must not change checks, scaffolds, grading, tutor output, or learner state, or delay the authoritative response. Bound tasks/deadlines, reserve budget, deduplicate local scheduling, copy immutable minimal state, and use independent database sessions. Recheck eligibility before dispatch; cancel/skip revoked, disconnected, late, or overloaded work. Prove shadow on/off behavioral equivalence, including retries and rollback. Local cancellation does not guarantee remote cancellation or billing deduplication. | [Phase 4](jev-implementation-plan.md#phase-4--read-only-shadow-intent-comparison), [shadow ownership](jev-architecture.md#existing-seam-and-ownership), [chat service](../app/services/chat.py) |
| S82 | Jev accounting and diagnostic lifecycle | Proposed | Phase 4 support: extend S47/S48/S61 with independent decision-call accounting, model/prompt/policy versions, task status, and traceable diagnostic retention/export/erasure. Enforce request, input-size, concurrency, and spend limits before dispatch. Preserve unknown usage/cost and potentially incurred calls after cancellation; suppress late or deleted diagnostic payloads without erasing accounting facts. Keep comparison records separate from learner observations and ordinary logs free of raw state. | [Phase 4](jev-implementation-plan.md#phase-4--read-only-shadow-intent-comparison), [call accounting](../app/services/llm_log.py), [budget service](../app/services/budget.py), [retention service](../app/services/retention.py) |
| S83 | Gated live intent experiment and rollback | Proposed | Phase 5: after reviewed quality/privacy results, define a bounded invited cohort, versioned policy, exposure/spend limits, stop rules, and rollback owner. Enable intent classification only; existing grading and evidence machinery applies afterward. Invalid, tied, uncertain, or failed results defer; withdrawal creates no failed observation. Do not silently add a second paid classifier. Test evidence idempotency and rollback to FAST; promotion requires an explicit evaluation decision. | [Phase 5](jev-implementation-plan.md#phase-5--bounded-enabled-intent-experiment), [fallback rules](jev-architecture.md#gates-boundaries-and-fallback), [rollout checklist](jev-implementation-plan.md#review-and-rollout-checklist) |
...
| S85 | Optional additional Jev decision studies | Proposed | Later opportunities, not pilot requirements: grounding sufficiency/stored-content routing (S26/S28), reviewed concept mapping (S24), inspectable preference suggestions (S02/S44), or a grading second opinion (S59). Give each a separate labeled evaluation and approval to expand scope. Require authorized candidates and abstention; do not automatically persist preferences, merge concept evidence, or average vendor scores into grades or ability estimates. | [Opportunity map](jev-architecture.md#ranked-opportunity-map), [scope alternatives](jev-implementation-plan.md#choice-of-scope) |
...
- **Backend suite not green:** `tests/eval/test_eval.py::test_retrieval_eval_gate` reported
  ...
  reported **1702 passed, 6 skipped**. This remains unresolved under S76.
