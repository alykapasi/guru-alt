...
**Status:** Implemented (branch `feat/s12-s23`; extended on branch `feat/s23-s62`) ·
**Priority:** High
...
**Implemented (twelfth pass) — the sacrificed prerequisite is now reported, not only logged.**
Plan generation's handling of a stored cycle was already right: drop whichever edge closes the
ring and order by what remains, because refusing to plan would strand the learner over an edge
they did not author. What was wrong is that the decision reached a log line and stopped there,
so a subject whose dependencies had been quietly sacrificed looked exactly like one with none.
`GET /subjects/{id}/prerequisite-conflicts` answers it, loading the same edges in the same
order and applying the same rule — the report cannot drift from what planning actually did,
because computing it a second way is precisely how the two would come to disagree.

Read-only on purpose. A cycle means two components each claim to come first, and which claim is
wrong is not something the graph knows; removing an edge is a curriculum decision that belongs
to a person. The endpoint puts it in front of one.

One boundary is pinned by a test rather than left to be rediscovered, because the first draft of
this got it wrong in the other direction: a ring closing through a KC in *another* subject is
never reported. `list_edges_for_subject` loads only edges whose dependent is in the subject, and
a ring needs every node to appear as a dependent, so such a ring never reaches the edge set.
...
ordering, which loads one subject's edges — that remains S24's, and the report inherits the same
blind spot. And the check is per-request, so two concurrent additions could still race a cycle
into existence between one's check and the other's insert; the unique constraint does not catch
that. Nothing surfaces the report in the product either — it is an endpoint, with no screen.
...
**Status:** Partially implemented (branch `fix/tracker-s51-s31`; extended on branch
`feat/s23-s62`) · **Priority:** Supporting
...
**Implemented (twelfth pass) — the transcript read is bounded.** `GET
/conversations/{id}/messages` returned every message a conversation had ever held: a query whose
cost grows with how much the learner has talked, with no ceiling. The conversation big enough to
break it is the one they are most invested in, so the failure arrives at the worst moment.

It pages now, and the bound is the default rather than something a caller opts into — a bound
you have to ask for does not bound anything, and the client that never passes `limit` is exactly
the one this exists for. An oversized `limit` is clamped instead of refused: a client asking past
the ceiling wants as much transcript as it can have, and a 422 is a worse answer than the
ceiling. `has_more` is answered by reading one row past the page rather than by counting the
transcript, since a count is a second full scan of the thing being avoided.

The cursor is `(created_at, id)`, not `created_at` — the transaction-clock finding from S56 and
S14 arriving in a third place. A turn's messages share one timestamp, so a timestamp-only cursor
would either step over the rest of a tied group or repeat it forever. A stale or foreign cursor
yields the newest page rather than an error, because a client holding a dead id wants a
transcript, not a broken screen.

This is deliberately a *different* bound from the turn window (S47): that one limits what is sent
to a model, is small, and is priced in tokens; this one limits what the client renders, is large,
and is priced in query and transfer. They are separate settings because collapsing them would
make either the turn ruinous or the transcript useless. The frontend pages backwards with a
cursor rather than re-requesting a larger window, which would re-read everything already on
screen — most of the cost this removes.

One existing test asserted the old premise — "the client still shows everything" — and was
...
report a number the evidence cannot support). The new page has no query-count budget asserting
it stays flat as a conversation grows, which is the measurement this item otherwise insists on.
Profile refresh still recomputes each dimension over the full history — S43 made it skip when
there is no new evidence, which is a different saving. Activity aggregation is still computed per
request rather than incrementally. And no other unbounded list was audited: this closed the one
the entry named, not the class.
